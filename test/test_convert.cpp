...
{
    wchar_t buf[50];
    TEST(nowide::widen(buf, 50, s.c_str()) == buf);
    return buf;
}
...
{
    char buf[50];
    TEST(nowide::narrow(buf, 50, s.c_str()) == buf);
    return buf;
}
...
{
    wchar_t buf[50];
    TEST(nowide::widen(buf, 50, s.c_str(), s.c_str() + s.size()) == buf);
    return buf;
}
...
{
    char buf[50];
    TEST(nowide::narrow(buf, 50, s.c_str(), s.c_str() + s.size()) == buf);
    return buf;
}

std::wstring widen_raw_string(const std::string& s)
{
    return nowide::widen(s.c_str());
}

std::string narrow_raw_string(const std::wstring& s)
{
    return nowide::narrow(s.c_str());
}
...
std::wstring widen_raw_string_and_size(const std::string& s)
{
    ...
    return nowide::widen(s2.c_str(), s.size());
}
...
std::string narrow_raw_string_and_size(const std::wstring& s)
{
    ...
    return nowide::narrow(s2.c_str(), s.size());
}

std::wstring widen_convert_buffer(const std::string& s)
{
    std::array<wchar_t, 200> out;
    const auto* result = nowide::utf::convert_buffer(out.data(), out.size(), s.data(), s.data() + s.size());
...
ASSERT_RETURN_TYPE(nowide::widen, char, std::wstring);
#ifdef __cpp_char8_t
ASSERT_RETURN_TYPE(nowide::widen, char8_t, std::wstring);
#endif
ASSERT_RETURN_TYPE(nowide::narrow, wchar_t, std::string);
ASSERT_RETURN_TYPE(nowide::narrow, char16_t, std::string);
ASSERT_RETURN_TYPE(nowide::narrow, char32_t, std::string);
...
void test_main(int, char**, char**)
{
    ...
    std::cout << "- nowide::widen" << std::endl;
    {
        ...
        TEST(nowide::widen(buf, 5, b, e - 1) == buf);
        TEST(buf == whello_3e);
        ...
        TEST(nowide::widen("\xd7\xa9\xd7\x9c\xd7\x95\xd7\x9d") == whello);
    }
    std::cout << "- nowide::narrow" << std::endl;
    {
        ...
        // Raw literals are also possible
        TEST_EQ(nowide::narrow(L"\u05e9\u05dc\u05d5\u05dd"), hello);
    }

    std::cout << "- nowide::utf::convert_buffer" << std::endl;
    {
    ...
    run_all(nowide::widen, nowide::narrow);
#ifdef NOWIDE_TEST_STD_STRINGVIEW
