...
{
    TEST(nowide::system(nullptr) != 0);
    const std::string command = "\"" + std::string(exe_path) + "\" " + example;
#if NOWIDE_TEST_USE_NARROW
    TEST_EQ(nowide::setenv("NOWIDE_TEST", example.c_str(), 1), 0);
    TEST_EQ(nowide::setenv("NOWIDE_TEST_NONE", example.c_str(), 1), 0);
    TEST_EQ(nowide::unsetenv("NOWIDE_TEST_NONE"), 0);
    TEST_EQ(nowide::setenv("NOWIDE_EMPTY", "", 1), 0);
    TEST(nowide::getenv("NOWIDE_EMPTY"));
    TEST_EQ(nowide::system(command.c_str()), 0);
    std::cout << "Parent ok" << std::endl;
#else
    const std::wstring envVar = L"NOWIDE_TEST=" + nowide::widen(example);
    TEST_EQ(_wputenv(envVar.c_str()), 0);
    const std::wstring wcommand = nowide::widen(command);
